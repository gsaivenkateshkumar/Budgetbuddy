...
3. Before the first deploy, fill in the env vars marked `sync: false` in
   ...
   - `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `GROQ_API_KEY` — only the one
     matching `AI_PROVIDER` is needed (set `AI_PROVIDER` to `openai`,
     `anthropic`, or `groq` to enable AI; `GROQ_MODEL` defaults to
     `openai/gpt-oss-120b` and rarely needs changing — Groq deprecates
     models periodically, so if Groq requests ever start 404ing again,
     check `console.groq.com/docs/deprecations` before assuming the
     endpoint is wrong)
